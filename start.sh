...
if [ "$DEV" = "1" ]; then
  ...
  start_go() {
    ...
    FLOFFI_ALLOW_DEV_BIND="${FLOFFI_ALLOW_DEV_BIND:-1}" FLOFFI_ALLOW_INSECURE_HTTP="${FLOFFI_ALLOW_INSECURE_HTTP:-1}" FLOFFI_DEV_UI="http://localhost:$VITE_PORT" ./floffi serve --port "$PORT" $DOTENV_OPT &
    GO_PID=$!
  }
...
echo
exec env FLOFFI_ALLOW_DEV_BIND="${FLOFFI_ALLOW_DEV_BIND:-1}" FLOFFI_ALLOW_INSECURE_HTTP="${FLOFFI_ALLOW_INSECURE_HTTP:-1}" ./floffi serve --port "$PORT" $DOTENV_OPT
